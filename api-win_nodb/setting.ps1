...
(Get-Content $replace_file) -replace 'HTTPC "(.*)" URI "(.*)//[^/]*/', ('HTTPC "$1" URI "$2//'+$HostName+':'+$port+'/') | out-file -encoding ASCII $replace_file

Write-Host "!!! Please copy folder printer to USB Disk folder /EXEC for printer install. !!!" -ForegroundColor green